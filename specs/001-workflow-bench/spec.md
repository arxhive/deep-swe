...
- **FR-025**: Before starting any run, the tool MUST verify that exactly one usable credential for driving the workflow is available - either an API key or a mintable OAuth-style token the owner can generate via the standard token-setup flow - and MUST refuse to start with a single clear actionable error (naming the accepted credential forms and how to obtain a token) when none is present. This validation MUST happen before any sandbox is provisioned, so a missing credential never wastes provisioning work.
