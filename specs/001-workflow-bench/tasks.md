...
- [ ] T034 [US2] Implement the `compare` subcommand in `bench/src/wfbench/cli.py` (per contracts/cli.md): repeatable `--command` (require >=2, else exit 2); identical other flags as `run`; build ONE `RunConfig` whose single resolved `Selection` and `--model` apply to every workflow (NFR-002). Reuse the `run` preflight path.
- [ ] T035 [US2] Implement compare orchestration in `bench/src/wfbench/runner.py`: `run_comparison(cfg) -> Comparison` that runs `run_workflow` for each `WorkflowRef` over the identical selection (sequential, continue-on-failure), writes a per-workflow `run-<slug>.json`/`report-<slug>.md`, then calls `build_comparison(runs)`.
...
- [ ] T037 [P] [US2] Extend `bench/tests/unit/test_cli.py`: `compare` requires >=2 `--command`; the same selection/model object is shared across workflows (assert on the constructed `RunConfig`, not on mocks).
